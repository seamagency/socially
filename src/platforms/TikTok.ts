...
export class TikTok implements SocialPlatform {
    ...
    generateAuthUrl(scopes: string[] = ['user.info.basic', 'video.publish', 'video.upload']): string {
    ...
    async post(content: PostContent): Promise<PostResult> {
        ...
        if (!videoUrl.startsWith('http')) {
            return {
                platform: this.name,
                success: false,
                error: 'TikTok integration currently only supports public URLs (PULL_FROM_URL).'
            };
        }
