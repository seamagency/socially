...
export class Facebook implements SocialPlatform {
  ...
  async exchangeCodeForToken(
    ...
    if (!this.clientId || !this.clientSecret || !this.redirectUri) {
      throw new Error(
        "Facebook OAuth requires clientId, clientSecret, and redirectUri",
      );
    }
  ...
  async refreshAccessToken(): Promise<string> {
    if (!this.accessToken || !this.clientId || !this.clientSecret) {
      throw new Error(
        "Facebook token refresh requires accessToken, clientId, and clientSecret",
      );
    }
  ...
  > {
    ...
    try {
      ...
      throw new Error(
        `Failed to get pages: ${error.response?.data?.error?.message || error.message}`,
      );
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (accountsResponse.data?.data?.length > 0) {
          ...
          if (!targetPage) {
            ...
            targetPage = pages[0];
            console.warn(
              `📘 Target Page ID ${this.pageId} not found/invalid. Defaulting to: ${targetPage.name}`,
            );
          }
        ...
        console.warn(
          "📘 Failed to fetch pages list. Trying with current token/ID as is (might fail if User Token).",
        );
      }
      ...
      if (content.media && content.media.length > 0) {
        ...
        if (!mediaPath.startsWith("http")) {
          throw new Error(
            "Facebook Graph API (Axios implementation) requires public URLs for media.",
          );
        }
