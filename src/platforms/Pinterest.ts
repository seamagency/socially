  ...
  async post(content: PostContent): Promise<PostResult> {
    ...
    if (!this.defaultBoardId) {
      return {
        ...
        success: false,
        error:
          "Pinterest posting requires a boardId (configure in SocialConfig).",
      };
    }
    ...
    try {
      ...
      if (isVideo) {
        ...
        const { upload_url, media_id, upload_parameters } =
        ...
        mediaSource = {
          ...
          media_id: media_id,
          cover_image_url:
            content.media[1] || "https://placehold.co/600x400.png", // Pinterest requires cover image for video
        };
      } else {
        // Image URL Flow
