...
export class Discord implements SocialPlatform {
   ...
   */
  static generateBotInviteUrl(
    clientId: string,
    permissions: number = 2048,
  ): string {
  ...
  > {
    try {
      ...
      throw new Error(
        `Failed to fetch guilds: ${error.response?.data?.message || error.message}`,
      );
    }
  }
  ...
  async getChannels(
    ...
    try {
      const response = await axios.get(
        `https://discord.com/api/guilds/${targetGuildId}/channels`,
        {
          headers: { Authorization: `Bot ${this.botToken}` },
        },
      );
      ...
      throw new Error(
        `Failed to fetch channels: ${error.response?.data?.message || error.message}`,
      );
    }
  }
  ...
  async post(content: PostContent, channelId?: string): Promise<PostResult> {
    ...
    try {
      ...
      const response = await axios.post(
        `https://discord.com/api/channels/${targetChannelId}/messages`,
        finalForm,
        {
          headers: {
    ...
    try {
      ...
      if (targetGuildId) {
        const guildResponse = await axios.get(
          `https://discord.com/api/guilds/${targetGuildId}?with_counts=true`,
          {
            headers: { Authorization: `Bot ${this.botToken}` },
          },
        );
