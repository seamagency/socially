...
export class Slack implements SocialPlatform {
  ...
  async exchangeCodeForToken(
    code: string,
  ): Promise<{ accessToken: string; teamId: string; botUserId: string }> {
    try {
      ...
      if (!response.data.ok) {
        throw new Error(
          response.data.error || "Failed to exchange code for token",
        );
      }
  ...
  async getChannels(): Promise<{ id: string; name: string }[]> {
    ...
    try {
      ...
      throw new Error(
        `Failed to fetch channels: ${error.response?.data?.error || error.message}`,
      );
    }
  }
