...
export class Twitter implements SocialPlatform {
  ...
  async refreshAccessToken(): Promise<string> {
    if (!this.refreshToken || !this.clientId) {
      throw new Error(
        "Twitter token refresh requires refreshToken and clientId",
      );
    }
  ...
  async post(content: PostContent): Promise<PostResult> {
    ...
    if (!this.oauth || !this.token) {
      return {
        ...
        success: false,
        error:
          "Twitter posting requires OAuth 1.0a credentials (apiKey, apiSecret, accessToken, accessSecret).",
      };
    }
      ...
      if (content.media && content.media.length > 0) {
        for (const mediaPath of content.media) {
          ...
          if (mediaPath.startsWith("http")) {
            const res = await axios.get(mediaPath, {
