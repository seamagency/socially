...
export class Telegram implements SocialPlatform {
  ...
  async getChatInfo(chatId: string): Promise<any> {
    try {
      ...
      throw new Error(
        `Failed to get chat info: ${error.response?.data?.description || error.message}`,
      );
    }
  }
  ...
  async post(content: PostContent, chatId?: string): Promise<PostResult> {
    ...
    try {
      ...
      // Multiple media (media group)
        ...
        const media = content.media.slice(0, 10).map((url, index) => {
          ...
          return {
            ...
            ...(index === 0 && content.text
              ? { caption: content.text, parse_mode: "HTML" }
              : {}),
          };
        });
        ...
        if (!response.data.ok) {
          throw new Error(
            response.data.description || "Failed to send media group",
          );
        }
  ...
  async getStats(): Promise<any> {
    ...
    try {
      ...
      if (!response.data.ok) {
        throw new Error(
          response.data.description || "Failed to get member count",
        );
      }
